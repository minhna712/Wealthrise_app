  ...
  return (
    <div style={{ minHeight:"100%", background:"#FFF9F3", fontFamily:"'Nunito', sans-serif" }}>
      ...
      <div style={{ padding:"0 20px" }}>
        {list.map(a => {
          const isSaved = userState.savedArticleIds.includes(a.id);
          return (
            <button key={a.id} onClick={() => onNavigateToArticle(a.id)} style={{
              ...
              <button onClick={e => {
                e.stopPropagation();
                isSaved ? unsaveArticle(a.id) : saveArticle(a.id);
              }} style={{ background:"none", border:"none", cursor:"pointer", padding:"4px", flexShrink:0 }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill={isSaved?"#D95C5C":"none"} stroke={isSaved?"#D95C5C":"#C0B8B0"} strokeWidth="2" strokeLinecap="round"><path d="M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z"/></svg>
              </button>
            </button>
          );
        })}
      ...
      <div style={{ margin:"8px 20px 24px", padding:"16px", borderRadius:"18px", background:"linear-gradient(120deg,#3A2E28,#5A4030)" }}>
        ...
        <button style={{ padding:"9px 20px", borderRadius:"12px", background:"#D95C5C", border:"none", cursor:"pointer", fontSize:"13px", fontWeight:700, color:"#FFFFFF", fontFamily:"'Nunito', sans-serif" }}>Xem thử thách →</button>
      </div>
    </div>
  );
}
